...
$app->post('/tags/add', function () use (&$app) {
    $user = wp_get_current_user();
    if (empty($user->ID)) {
        return json_response(array('result' => 'login'));
    }
    ...
    if (!count($ids)) {
        return json_response(array('result' => 'ok'));
    }
...
$app->post('/tags/remove', function () use (&$app) {
    ...
    return json_response(array('tag' => $tagId, 'word' => $wordId));
});
...
$app->post('/picture/add', function () use (&$app) {
    ...
    if (empty($word) || empty($url) || empty($thumbUrl)) {
        return json_response(array('result' => 'bad-word'));
    }

    if (!preg_match('/^image\//', tiliKgGetMime($url)) || !preg_match('/^image\//', tiliKgGetMime($thumbUrl))) {
        return json_response(array('result' => 'bad-image'));
    }
    ...
    return json_response(array('result' => 'ok', 'id' => $db->lastInsertId()));
});
