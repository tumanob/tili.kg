...
class ApeTag
{
	...
	public function GenerateAPEtag() {
		...
		foreach ($this->tag_data as $key => $arrayofvalues) {
			...
			$tagitem  = Utils::LittleEndian2String(strlen($valuestring), 4);
	...
	public function GenerateAPEtagHeaderFooter(&$items, $isheader=false) {
		...
		$APEheader  = 'APETAGEX';
		$APEheader .= Utils::LittleEndian2String(2000, 4);
		$APEheader .= Utils::LittleEndian2String(32 + $tagdatalength, 4);
		$APEheader .= Utils::LittleEndian2String(count($items), 4);
