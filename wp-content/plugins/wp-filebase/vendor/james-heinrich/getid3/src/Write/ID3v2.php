...
use JamesHeinrich\GetID3\Module\Tag\ID3v2 as Tag;
...
class ID3v2
{
	...
	public function WriteID3v2() {
		...
		if (!empty($this->filename) && (is_writeable($this->filename) || (!file_exists($this->filename) && is_writeable(dirname($this->filename))))) {
			...
			if (!Utils::intValueSupported($OldThisFileInfo['filesize'])) {
			...
			switch ($frame_name) {
				...
				case 'TXXX':
					...
					if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'], $this->majorversion)) {
						...
						$framedata .= $source_data_array['description'] . Tag::TextEncodingTerminatorLookup($source_data_array['encodingid']);
						...
						foreach ($source_data_array as $key => $val) {
							if (!$this->ID3v2IsValidETCOevent($val['typeid'])) {
								...
								if (($val['timestamp'] > 0) && ($previousETCOtimestamp >= $val['timestamp'])) {
									...
									$framedata .= Utils::BigEndian2String($val['timestamp'], 4, false);
								}
							}
						}
					}
				...
				case 'MLLT':
					...
					if (($source_data_array['framesbetweenreferences'] > 0) && ($source_data_array['framesbetweenreferences'] <= 65535)) {
						$framedata .= Utils::BigEndian2String($source_data_array['framesbetweenreferences'], 2, false);
					} else {
					...
					if (($source_data_array['bytesbetweenreferences'] > 0) && ($source_data_array['bytesbetweenreferences'] <= 16777215)) {
						$framedata .= Utils::BigEndian2String($source_data_array['bytesbetweenreferences'], 3, false);
					} else {
					...
					if (($source_data_array['msbetweenreferences'] > 0) && ($source_data_array['msbetweenreferences'] <= 16777215)) {
						$framedata .= Utils::BigEndian2String($source_data_array['msbetweenreferences'], 3, false);
					} else {
					...
					foreach ($source_data_array as $key => $val) {
						if (($key != 'framesbetweenreferences') && ($key != 'bytesbetweenreferences') && ($key != 'msbetweenreferences') && ($key != 'bitsforbytesdeviation') && ($key != 'bitsformsdeviation') && ($key != 'flags')) {
							$unwrittenbitstream .= str_pad(Utils::Dec2Bin($val['bytedeviation']), $source_data_array['bitsforbytesdeviation'], '0', STR_PAD_LEFT);
							$unwrittenbitstream .= str_pad(Utils::Dec2Bin($val['msdeviation']),   $source_data_array['bitsformsdeviation'],    '0', STR_PAD_LEFT);
						}
					}
				...
				case 'USLT':
					...
					if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'])) {
						$this->errors[] = 'Invalid Text Encoding in '.$frame_name.' ('.$source_data_array['encodingid'].') for ID3v2.'.$this->majorversion;
					} elseif (Tag::LanguageLookup($source_data_array['language'], true) == '') {
				...
				case 'SYLT':
					...
					if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'])) {
						...
						foreach ($source_data_array['data'] as $key => $val) {
							$framedata .= $val['data'] . Tag::TextEncodingTerminatorLookup($source_data_array['encodingid']);
							$framedata .= Utils::BigEndian2String($val['timestamp'], 4, false);
						}
					}
					break;
				...
				case 'RVA2':
					...
					foreach ($source_data_array as $key => $val) {
						if ($key != 'description') {
							...
							$framedata .= Utils::BigEndian2String($val['volumeadjust'], 2, false, true); // signed 16-bit
							if (!$this->IsWithinBitRange($source_data_array['bitspeakvolume'], 8, false)) {
								$framedata .= chr($val['bitspeakvolume']);
								if ($val['bitspeakvolume'] > 0) {
									$framedata .= Utils::BigEndian2String($val['peakvolume'], ceil($val['bitspeakvolume'] / 8), false, false);
								}
							} else {
				...
				case 'RVAD':
					...
					if (!$this->IsWithinBitRange($source_data_array['bitsvolume'], 8, false)) {
						...
						$framedata .= Utils::BigEndian2String($source_data_array['volumechange']['right'], ceil($source_data_array['bitsvolume'] / 8), false);
						$framedata .= Utils::BigEndian2String($source_data_array['volumechange']['left'],  ceil($source_data_array['bitsvolume'] / 8), false);
						$framedata .= Utils::BigEndian2String($source_data_array['peakvolume']['right'], ceil($source_data_array['bitsvolume'] / 8), false);
						$framedata .= Utils::BigEndian2String($source_data_array['peakvolume']['left'],  ceil($source_data_array['bitsvolume'] / 8), false);
						if ($source_data_array['volumechange']['rightrear'] || $source_data_array['volumechange']['leftrear'] ||
							...
							$source_data_array['volumechange']['bass'] || $source_data_array['peakvolume']['bass']) {
								$framedata .= Utils::BigEndian2String($source_data_array['volumechange']['rightrear'], ceil($source_data_array['bitsvolume']/8), false);
								$framedata .= Utils::BigEndian2String($source_data_array['volumechange']['leftrear'],  ceil($source_data_array['bitsvolume']/8), false);
								$framedata .= Utils::BigEndian2String($source_data_array['peakvolume']['rightrear'], ceil($source_data_array['bitsvolume']/8), false);
								$framedata .= Utils::BigEndian2String($source_data_array['peakvolume']['leftrear'],  ceil($source_data_array['bitsvolume']/8), false);
						}
						if ($source_data_array['volumechange']['center'] || $source_data_array['peakvolume']['center'] ||
							$source_data_array['volumechange']['bass'] || $source_data_array['peakvolume']['bass']) {
								$framedata .= Utils::BigEndian2String($source_data_array['volumechange']['center'], ceil($source_data_array['bitsvolume']/8), false);
								$framedata .= Utils::BigEndian2String($source_data_array['peakvolume']['center'], ceil($source_data_array['bitsvolume']/8), false);
						}
						if ($source_data_array['volumechange']['bass'] || $source_data_array['peakvolume']['bass']) {
								$framedata .= Utils::BigEndian2String($source_data_array['volumechange']['bass'], ceil($source_data_array['bitsvolume']/8), false);
								$framedata .= Utils::BigEndian2String($source_data_array['peakvolume']['bass'], ceil($source_data_array['bitsvolume']/8), false);
						}
					}
					break;
				...
				case 'EQU2':
					...
					if (($source_data_array['interpolationmethod'] < 0) || ($source_data_array['interpolationmethod'] > 1)) {
						...
						$framedata .= str_replace("\x00", '', $source_data_array['description'])."\x00";
						foreach ($source_data_array['data'] as $key => $val) {
							$framedata .= Utils::BigEndian2String(intval(round($key * 2)), 2, false);
							$framedata .= Utils::BigEndian2String($val, 2, false, true); // signed 16-bit
						}
					}
					break;
						...
						foreach ($source_data_array as $key => $val) {
							if ($key != 'bitsvolume') {
								if (($key > 32767) || ($key < 0)) {
									...
									$framedata .= Utils::BigEndian2String($key, 2, false);
									$framedata .= Utils::BigEndian2String($val, ceil($source_data_array['adjustmentbits'] / 8), false);
								}
							}
						}
					}
				...
				case 'RVRB':
					...
					if (!$this->IsWithinBitRange($source_data_array['left'], 16, false)) {
						...
						$framedata .= Utils::BigEndian2String($source_data_array['left'], 2, false);
						$framedata .= Utils::BigEndian2String($source_data_array['right'], 2, false);
				...
				case 'APIC':
					...
					if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'])) {
						...
						$framedata .= (!empty($source_data_array['description']) ? $source_data_array['description'] : '') . Tag::TextEncodingTerminatorLookup($source_data_array['encodingid']);
						$framedata .= $source_data_array['data'];
					}
					break;
				...
				case 'GEOB':
					...
					if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'])) {
						...
						$framedata .= chr($source_data_array['encodingid']);
						$framedata .= str_replace("\x00", '', $source_data_array['mime'])."\x00";
						$framedata .= $source_data_array['filename'] . Tag::TextEncodingTerminatorLookup($source_data_array['encodingid']);
						$framedata .= $source_data_array['description'] . Tag::TextEncodingTerminatorLookup($source_data_array['encodingid']);
						$framedata .= $source_data_array['data'];
					}
					break;
				...
				case 'PCNT':
					...
					$framedata .= Utils::BigEndian2String($source_data_array['data'], 4, false);
					break;
				...
				case 'RBUF':
					...
					if (!$this->IsWithinBitRange($source_data_array['buffersize'], 24, false)) {
						...
						$framedata .= Utils::BigEndian2String($source_data_array['buffersize'], 3, false);
						$flag .= '0000000';
						$flag .= $source_data_array['flags']['embededinfo'] ? '1' : '0';
						$framedata .= chr(bindec($flag));
						$framedata .= Utils::BigEndian2String($source_data_array['nexttagoffset'], 4, false);
					}
					break;
				...
				case 'AENC':
					...
					if (!$this->IsWithinBitRange($source_data_array['previewstart'], 16, false)) {
						...
						$framedata .= str_replace("\x00", '', $source_data_array['ownerid'])."\x00";
						$framedata .= Utils::BigEndian2String($source_data_array['previewstart'], 2, false);
						$framedata .= Utils::BigEndian2String($source_data_array['previewlength'], 2, false);
				...
				case 'LINK':
					...
					if (!Tag::IsValidID3v2FrameName($source_data_array['frameid'], $this->majorversion)) {
					...
					} elseif (($source_data_array['frameid'] == 'USER') && (Tag::LanguageLookup($source_data_array['additionaldata'], true) == '')) {
					...
					} elseif ((($source_data_array['frameid'] == 'COMM') || ($source_data_array['frameid'] == 'SYLT') || ($source_data_array['frameid'] == 'USLT')) && ((Tag::LanguageLookup(substr($source_data_array['additionaldata'], 0, 3), true) == '') || (substr($source_data_array['additionaldata'], 3) == ''))) {
				...
				case 'POSS':
					...
					if (($source_data_array['timestampformat'] < 1) || ($source_data_array['timestampformat'] > 2)) {
						...
						$framedata .= chr($source_data_array['timestampformat']);
						$framedata .= Utils::BigEndian2String($source_data_array['position'], 4, false);
					}
					break;
				...
				case 'COMR':
					...
					if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'])) {
						...
						$framedata .= $source_data_array['sellername'] . Tag::TextEncodingTerminatorLookup($source_data_array['encodingid']);
						$framedata .= $source_data_array['description'] . Tag::TextEncodingTerminatorLookup($source_data_array['encodingid']);
				...
				case 'ASPI':
					...
					if (!$this->IsWithinBitRange($source_data_array['datastart'], 32, false)) {
						...
						$framedata .= Utils::BigEndian2String($source_data_array['datastart'], 4, false);
						$framedata .= Utils::BigEndian2String($source_data_array['datalength'], 4, false);
						$framedata .= Utils::BigEndian2String($source_data_array['indexpoints'], 2, false);
						$framedata .= Utils::BigEndian2String($source_data_array['bitsperpoint'], 1, false);
						foreach ($source_data_array['indexes'] as $key => $val) {
							$framedata .= Utils::BigEndian2String($val, ceil($source_data_array['bitsperpoint'] / 8), false);
						}
					}
					break;
				...
				case 'RGAD':
					...
					if (($source_data_array['track_adjustment'] > 51) || ($source_data_array['track_adjustment'] < -51)) {
						...
						$framedata .= Utils::Float2String($source_data_array['peakamplitude'], 32);
						$framedata .= Utils::RGADgainString($source_data_array['raw']['track_name'], $source_data_array['raw']['track_originator'], $source_data_array['track_adjustment']);
						$framedata .= Utils::RGADgainString($source_data_array['raw']['album_name'], $source_data_array['raw']['album_originator'], $source_data_array['album_adjustment']);
					}
					break;
	...
	public function GenerateID3v2Tag($noerrorsonly=true) {
		...
		if (is_array($this->tag_data)) {
						...
						if ($this->ID3v2FrameIsAllowed($frame_name, $source_data_array)) {
							...
							if ($frame_data = $this->GenerateID3v2FrameData($frame_name, $source_data_array)) {
								...
								if ($this->majorversion >= 4) {
									...
									$frame_length = Utils::BigEndian2String(strlen($frame_data), 4, true);
								} else {
									$frame_length = Utils::BigEndian2String(strlen($frame_data), 4, false);
								}
			...
			while ($this->paddedlength < (strlen($tagstring) + Tag::ID3v2HeaderLength($this->majorversion))) {
				$this->paddedlength += 1024;
			}

			$footer = false; // ID3v2 footers not yet supported in getID3()
			if (!$footer && ($this->paddedlength > (strlen($tagstring) + Tag::ID3v2HeaderLength($this->majorversion)))) {
				...
				if (($this->paddedlength - strlen($tagstring) - Tag::ID3v2HeaderLength($this->majorversion)) > 0) {
					$tagstring .= str_repeat("\x00", $this->paddedlength - strlen($tagstring) - Tag::ID3v2HeaderLength($this->majorversion));
				}
			}
			...
			$tagheader .= Utils::BigEndian2String(strlen($tagstring), 4, true);
