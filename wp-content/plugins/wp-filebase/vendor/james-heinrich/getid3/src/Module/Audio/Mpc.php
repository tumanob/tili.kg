...
class Mpc extends \JamesHeinrich\GetID3\Module\Handler
{
	...
	public function ParseMPCsv7() {
		...
		$StreamVersionByte                           = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 1));
		$offset += 1;
		...
		$thisfile_mpc_header['frame_count']          = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 4));
		$offset += 4;
		...
		$FlagsDWORD1                                   = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 4));
		$offset += 4;
		...
		$thisfile_mpc_header['raw']['title_peak']      = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 2));
		$offset += 2;
		$thisfile_mpc_header['raw']['title_gain']      = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 2), true);
		$offset += 2;

		$thisfile_mpc_header['raw']['album_peak']      = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 2));
		$offset += 2;
		$thisfile_mpc_header['raw']['album_gain']      = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 2), true);
		$offset += 2;

		$FlagsDWORD2                                   = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 4));
		$offset += 4;
		...
		$thisfile_mpc_header['raw']['not_sure_what']   = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 3));
		$offset += 3;
		$thisfile_mpc_header['raw']['encoder_version'] = Utils::LittleEndian2Int(substr($MPCheaderData, $offset, 1));
		$offset += 1;
		...
		if ($thisfile_mpc_header['title_peak'] > 0) {
			...
			$info['replay_gain']['track']['peak'] = Utils::CastAsInt(round($thisfile_mpc_header['max_level'] * 1.18)); // why? I don't know - see mppdec.c
		}
