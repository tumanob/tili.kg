				...
				while ($nextRIFFoffset < min($info['filesize'], $info['avdataend'])) {
					...
					switch ($nextRIFFheaderID) {
						...
						default:
							...
							$this->warning('Expecting "RIFF|JUNK|IDVX" at '.$nextRIFFoffset.', found "'.$nextRIFFheaderID.'" ('.Utils::PrintHexBytes($nextRIFFheaderID).') - skipping rest of file');
							break 2;
		...
		switch ($RIFFsubtype) {
			...
			case 'WAVE':
				...
				if (isset($thisfile_riff_WAVE['rgad'][0]['data'])) {
					...
					$thisfile_riff_raw_rgad['fPeakAmplitude']      = Utils::LittleEndian2Float(substr($rgadData, 0, 4));
					...
					$nRadioRgAdjustBitstring      = str_pad(Utils::Dec2Bin($thisfile_riff_raw_rgad['nRadioRgAdjust']), 16, '0', STR_PAD_LEFT);
					$nAudiophileRgAdjustBitstring = str_pad(Utils::Dec2Bin($thisfile_riff_raw_rgad['nAudiophileRgAdjust']), 16, '0', STR_PAD_LEFT);
					$thisfile_riff_raw_rgad_track['name']       = Utils::Bin2Dec(substr($nRadioRgAdjustBitstring, 0, 3));
					$thisfile_riff_raw_rgad_track['originator'] = Utils::Bin2Dec(substr($nRadioRgAdjustBitstring, 3, 3));
					$thisfile_riff_raw_rgad_track['signbit']    = Utils::Bin2Dec(substr($nRadioRgAdjustBitstring, 6, 1));
					$thisfile_riff_raw_rgad_track['adjustment'] = Utils::Bin2Dec(substr($nRadioRgAdjustBitstring, 7, 9));
					$thisfile_riff_raw_rgad_album['name']       = Utils::Bin2Dec(substr($nAudiophileRgAdjustBitstring, 0, 3));
					$thisfile_riff_raw_rgad_album['originator'] = Utils::Bin2Dec(substr($nAudiophileRgAdjustBitstring, 3, 3));
					$thisfile_riff_raw_rgad_album['signbit']    = Utils::Bin2Dec(substr($nAudiophileRgAdjustBitstring, 6, 1));
					$thisfile_riff_raw_rgad_album['adjustment'] = Utils::Bin2Dec(substr($nAudiophileRgAdjustBitstring, 7, 9));
					...
					if (($thisfile_riff_raw_rgad_track['name'] != 0) && ($thisfile_riff_raw_rgad_track['originator'] != 0)) {
						$thisfile_riff['rgad']['track']['name']            = Utils::RGADnameLookup($thisfile_riff_raw_rgad_track['name']);
						$thisfile_riff['rgad']['track']['originator']      = Utils::RGADoriginatorLookup($thisfile_riff_raw_rgad_track['originator']);
						$thisfile_riff['rgad']['track']['adjustment']      = Utils::RGADadjustmentLookup($thisfile_riff_raw_rgad_track['adjustment'], $thisfile_riff_raw_rgad_track['signbit']);
					}
					if (($thisfile_riff_raw_rgad_album['name'] != 0) && ($thisfile_riff_raw_rgad_album['originator'] != 0)) {
						$thisfile_riff['rgad']['album']['name']       = Utils::RGADnameLookup($thisfile_riff_raw_rgad_album['name']);
						$thisfile_riff['rgad']['album']['originator'] = Utils::RGADoriginatorLookup($thisfile_riff_raw_rgad_album['originator']);
						$thisfile_riff['rgad']['album']['adjustment'] = Utils::RGADadjustmentLookup($thisfile_riff_raw_rgad_album['adjustment'], $thisfile_riff_raw_rgad_album['signbit']);
					}
				}
				...
				if (!empty($thisfile_riff_raw['fmt ']['nAvgBytesPerSec'])) {
					$thisfile_audio['bitrate'] = Utils::CastAsInt($thisfile_riff_raw['fmt ']['nAvgBytesPerSec'] * 8);
				}
				...
				if (isset($thisfile_riff_WAVE['bext'][0]['data'])) {
					...
					$thisfile_riff_WAVE_bext_0['bwf_version']    = Utils::LittleEndian2Int(substr($thisfile_riff_WAVE_bext_0['data'], 346,   1));
				...
				if (isset($thisfile_riff_WAVE['MEXT'][0]['data'])) {
					...
					$thisfile_riff_WAVE_MEXT_0['raw']['sound_information']      = Utils::LittleEndian2Int(substr($thisfile_riff_WAVE_MEXT_0['data'], 0, 2));
					...
					if ($thisfile_riff_WAVE_MEXT_0['flags']['homogenous']) {
						...
						$thisfile_riff_WAVE_MEXT_0['nominal_frame_size']        = Utils::LittleEndian2Int(substr($thisfile_riff_WAVE_MEXT_0['data'], 2, 2));
					}
					$thisfile_riff_WAVE_MEXT_0['anciliary_data_length']         = Utils::LittleEndian2Int(substr($thisfile_riff_WAVE_MEXT_0['data'], 6, 2));
					$thisfile_riff_WAVE_MEXT_0['raw']['anciliary_data_def']     = Utils::LittleEndian2Int(substr($thisfile_riff_WAVE_MEXT_0['data'], 8, 2));
				...
				if (isset($thisfile_riff_WAVE['cart'][0]['data'])) {
					...
					$thisfile_riff_WAVE_cart_0['zero_db_reference']    = Utils::LittleEndian2Int(substr($thisfile_riff_WAVE_cart_0['data'], 680,  4), true);
					for ($i = 0; $i < 8; $i++) {
						$thisfile_riff_WAVE_cart_0['post_time'][$i]['usage_fourcc'] =                  substr($thisfile_riff_WAVE_cart_0['data'], 684 + ($i * 8), 4);
						$thisfile_riff_WAVE_cart_0['post_time'][$i]['timer_value']  = Utils::LittleEndian2Int(substr($thisfile_riff_WAVE_cart_0['data'], 684 + ($i * 8) + 4, 4));
					}
				...
				if (isset($thisfile_riff_WAVE['SNDM'][0]['data'])) {
					...
					while ($SNDM_startoffset < $SNDM_endoffset) {
						...
						$SNDM_thisTagSize      = Utils::BigEndian2Int(substr($thisfile_riff_WAVE_SNDM_0_data, $SNDM_startoffset + $SNDM_thisTagOffset, 4));
						$SNDM_thisTagOffset += 4;
						$SNDM_thisTagKey       =                           substr($thisfile_riff_WAVE_SNDM_0_data, $SNDM_startoffset + $SNDM_thisTagOffset, 4);
						$SNDM_thisTagOffset += 4;
						$SNDM_thisTagDataSize  = Utils::BigEndian2Int(substr($thisfile_riff_WAVE_SNDM_0_data, $SNDM_startoffset + $SNDM_thisTagOffset, 2));
						$SNDM_thisTagOffset += 2;
						$SNDM_thisTagDataFlags = Utils::BigEndian2Int(substr($thisfile_riff_WAVE_SNDM_0_data, $SNDM_startoffset + $SNDM_thisTagOffset, 2));
						$SNDM_thisTagOffset += 2;
				...
				if (isset($thisfile_riff_WAVE['iXML'][0]['data'])) {
					...
					if ($parsedXML = Utils::XML2array($thisfile_riff_WAVE['iXML'][0]['data'])) {
				...
				if (!empty($info['wavpack'])) {
					...
					$OrignalRIFFheaderSize = Utils::LittleEndian2Int(substr($RIFFdata,  4, 4)) +  8;
					$OrignalRIFFdataSize   = Utils::LittleEndian2Int(substr($RIFFdata, 40, 4)) + 44;
					...
					$getid3_riff = new Riff($this->getid3);
					...
					switch ($thisfile_riff_raw['fmt ']['wFormatTag']) {
						...
						case 0x08AE: // ClearJump LiteWave
							...
							foreach ($flags as $flag => $length) {
								$riff_litewave_raw[$flag] = Utils::LittleEndian2Int(substr($thisfile_riff_WAVE['fmt '][0]['data'], $litewave_offset, $length));
			...
			case 'AIFC':
				...
				if (isset($thisfile_riff[$RIFFsubtype]['COMM'][0]['data'])) {
					...
					$thisfile_riff_audio['channels']         =         Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_COMM_0_data,  0,  2), true);
					$thisfile_riff_audio['total_samples']    =         Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_COMM_0_data,  2,  4), false);
					$thisfile_riff_audio['bits_per_sample']  =         Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_COMM_0_data,  6,  2), true);
					$thisfile_riff_audio['sample_rate']      = (int) Utils::BigEndian2Float(substr($thisfile_riff_RIFFsubtype_COMM_0_data,  8, 10));
					...
					if ($thisfile_riff[$RIFFsubtype]['COMM'][0]['size'] > 18) {
						...
						$CodecNameSize                       =         Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_COMM_0_data, 22,  1), false);
				...
				if (isset($thisfile_riff[$RIFFsubtype]['COMT'])) {
					$offset = 0;
					$CommentCount                                   = Utils::BigEndian2Int(substr($thisfile_riff[$RIFFsubtype]['COMT'][0]['data'], $offset, 2), false);
					$offset += 2;
					for ($i = 0; $i < $CommentCount; $i++) {
						$info['comments_raw'][$i]['timestamp']      = Utils::BigEndian2Int(substr($thisfile_riff[$RIFFsubtype]['COMT'][0]['data'], $offset, 4), false);
						$offset += 4;
						$info['comments_raw'][$i]['marker_id']      = Utils::BigEndian2Int(substr($thisfile_riff[$RIFFsubtype]['COMT'][0]['data'], $offset, 2), true);
						$offset += 2;
						$CommentLength                              = Utils::BigEndian2Int(substr($thisfile_riff[$RIFFsubtype]['COMT'][0]['data'], $offset, 2), false);
						$offset += 2;
						$info['comments_raw'][$i]['comment']        =                           substr($thisfile_riff[$RIFFsubtype]['COMT'][0]['data'], $offset, $CommentLength);
						$offset += $CommentLength;

						$info['comments_raw'][$i]['timestamp_unix'] = Utils::DateMac2Unix($info['comments_raw'][$i]['timestamp']);
...
*/
			...
			case '8SVX':
				...
				if (isset($thisfile_riff[$RIFFsubtype]['VHDR'][0]['offset'])) {
					...
					$thisfile_riff_RIFFsubtype_VHDR_0['oneShotHiSamples']  =   Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_VHDR_0['data'],  0, 4));
					$thisfile_riff_RIFFsubtype_VHDR_0['repeatHiSamples']   =   Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_VHDR_0['data'],  4, 4));
					$thisfile_riff_RIFFsubtype_VHDR_0['samplesPerHiCycle'] =   Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_VHDR_0['data'],  8, 4));
					$thisfile_riff_RIFFsubtype_VHDR_0['samplesPerSec']     =   Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_VHDR_0['data'], 12, 2));
					$thisfile_riff_RIFFsubtype_VHDR_0['ctOctave']          =   Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_VHDR_0['data'], 14, 1));
					$thisfile_riff_RIFFsubtype_VHDR_0['sCompression']      =   Utils::BigEndian2Int(substr($thisfile_riff_RIFFsubtype_VHDR_0['data'], 15, 1));
					$thisfile_riff_RIFFsubtype_VHDR_0['Volume']            = Utils::FixedPoint16_16(substr($thisfile_riff_RIFFsubtype_VHDR_0['data'], 16, 4));
				...
				if (isset($thisfile_riff[$RIFFsubtype]['CHAN'][0]['data'])) {
					$ChannelsIndex = Utils::BigEndian2Int(substr($thisfile_riff[$RIFFsubtype]['CHAN'][0]['data'], 0, 4));
	...
	public function ParseRIFFAMV($startoffset, $maxoffset) {
		...
		try {
			...
			$RIFFchunk['amvh']['reserved0']      = Utils::LittleEndian2Int(substr($AMVheader,  56,  4)); // 1? reserved?
			$RIFFchunk['amvh']['reserved1']      = Utils::LittleEndian2Int(substr($AMVheader,  60,  4)); // 0? reserved?
					...
					case 'LIST':
						...
						if (preg_match('#^(movi|rec )$#i', $listname)) {
							...
							if (!$FoundAllChunksWeNeed) {
								...
								$AudioChunkSize       = Utils::LittleEndian2Int(substr($AudioChunkHeader, 4, 4));
					...
					default:
						...
						switch ($chunkname) {
							case 'data':
								...
								if (preg_match('/^\xFF[\xE2-\xE7\xF2-\xF7\xFA-\xFF][\x00-\xEB]/s', substr($testData, 0, 4))) {
								...
								} elseif (preg_match('/^('.implode('|', array_map('preg_quote', Dts::$syncwords)).')/', $testData)) {
									...
									$info['wavpack']['size']   = Utils::LittleEndian2Int(substr($testData, 4, 4));
	...
	public function parseWavPackHeader($WavPackChunkData) {
		...
		$thisfile_wavpack['version']           = Utils::LittleEndian2Int(substr($WavPackChunkData,  0, 2));
		if ($thisfile_wavpack['version'] >= 2) {
			$thisfile_wavpack['bits']          = Utils::LittleEndian2Int(substr($WavPackChunkData,  2, 2));
		}
		if ($thisfile_wavpack['version'] >= 3) {
			$thisfile_wavpack['flags_raw']     = Utils::LittleEndian2Int(substr($WavPackChunkData,  4, 2));
			$thisfile_wavpack['shift']         = Utils::LittleEndian2Int(substr($WavPackChunkData,  6, 2));
			$thisfile_wavpack['total_samples'] = Utils::LittleEndian2Int(substr($WavPackChunkData,  8, 4));
			$thisfile_wavpack['crc1']          = Utils::LittleEndian2Int(substr($WavPackChunkData, 12, 4));
			$thisfile_wavpack['crc2']          = Utils::LittleEndian2Int(substr($WavPackChunkData, 16, 4));
			$thisfile_wavpack['extension']     =                              substr($WavPackChunkData, 20, 4);
			$thisfile_wavpack['extra_bc']      = Utils::LittleEndian2Int(substr($WavPackChunkData, 24, 1));
			for ($i = 0; $i <= 2; $i++) {
				$thisfile_wavpack['extras'][]  = Utils::LittleEndian2Int(substr($WavPackChunkData, 25 + $i, 1));
			}
