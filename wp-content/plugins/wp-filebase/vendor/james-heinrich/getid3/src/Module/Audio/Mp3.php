...
class Mp3 extends \JamesHeinrich\GetID3\Module\Handler
{
	...
	public function decodeMPEGaudioHeader($offset, &$info, $recursivesearch=true, $ScanAsCBR=false, $FastMPEGheaderScan=false) {
		...
		if ($MPEGaudioHeaderValidCache[$head4]) {
			...
			$info['error'][] = 'Invalid MPEG audio header ('.Utils::PrintHexBytes($head4).') at offset '.$offset;
			return false;
		}
		...
		if (!$FastMPEGheaderScan) {
			...
			if ($thisfile_mpeg_audio['protection']) {
				$thisfile_mpeg_audio['crc'] = Utils::BigEndian2Int(substr($headerstring, 4, 2));
			}
		}
		...
		if (substr($headerstring, 4 + 32, 4) == 'VBRI') {
			...
			$thisfile_mpeg_audio['VBR_encoder_version']     = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset +  4, 2)); // VbriVersion
			$thisfile_mpeg_audio['VBR_encoder_delay']       = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset +  6, 2)); // VbriDelay
			$thisfile_mpeg_audio['VBR_quality']             = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset +  8, 2)); // VbriQuality
			$thisfile_mpeg_audio['VBR_bytes']               = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 10, 4)); // VbriStreamBytes
			$thisfile_mpeg_audio['VBR_frames']              = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 14, 4)); // VbriStreamFrames
			$thisfile_mpeg_audio['VBR_seek_offsets']        = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 18, 2)); // VbriTableSize
			$thisfile_mpeg_audio['VBR_seek_scale']          = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 20, 2)); // VbriTableScale
			$thisfile_mpeg_audio['VBR_entry_bytes']         = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 22, 2)); // VbriEntryBytes
			$thisfile_mpeg_audio['VBR_entry_frames']        = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset + 24, 2)); // VbriEntryFrames

			$ExpectedNumberOfAudioBytes = $thisfile_mpeg_audio['VBR_bytes'];

			$previousbyteoffset = $offset;
			for ($i = 0; $i < $thisfile_mpeg_audio['VBR_seek_offsets']; $i++) {
				$Fraunhofer_OffsetN = Utils::BigEndian2Int(substr($headerstring, $FraunhoferVBROffset, $thisfile_mpeg_audio['VBR_entry_bytes']));
...
//				}
				...
				$thisfile_mpeg_audio['xing_flags_raw'] = Utils::BigEndian2Int(substr($headerstring, $VBRidOffset + 4, 4));
				...
				if ($thisfile_mpeg_audio['xing_flags']['frames']) {
					$thisfile_mpeg_audio['VBR_frames'] = Utils::BigEndian2Int(substr($headerstring, $VBRidOffset +  8, 4));
					//$thisfile_mpeg_audio['VBR_frames']--; // don't count header Xing/Info frame
				}
				if ($thisfile_mpeg_audio['xing_flags']['bytes']) {
					$thisfile_mpeg_audio['VBR_bytes']  = Utils::BigEndian2Int(substr($headerstring, $VBRidOffset + 12, 4));
				}
				...
				if ($thisfile_mpeg_audio['xing_flags']['vbr_scale']) {
					$thisfile_mpeg_audio['VBR_scale'] = Utils::BigEndian2Int(substr($headerstring, $VBRidOffset + 116, 4));
				}
		...
		//
		//    $SideInfoBitstream = Utils::BigEndian2Bin($SideInfoData);
		//    $SideInfoOffset = 0;
		//
	...
	public function FreeFormatFrameLength($offset, $deepscan=false) {
		...
		if (!$framelength) {
			...
			if (!$framelength) {
				$info['error'][] = 'Cannot find next free-format synch pattern ('.Utils::PrintHexBytes($SyncPattern1).' or '.Utils::PrintHexBytes($SyncPattern2).') after offset '.$offset;
				return false;
			} else {
	...
	public function getOnlyMPEGaudioInfoBruteForce() {
				...
				if ($MPEGaudioHeaderLengthCache[$head4] > 4) {
					...
					if ($next4{0} == "\xFF") {
						...
						if ($MPEGaudioHeaderValidCache[$next4]) {
							...
							Utils::safe_inc($Distribution['bitrate'][$LongMPEGbitrateLookup[$head4]]);
							Utils::safe_inc($Distribution['layer'][$LongMPEGlayerLookup[$head4]]);
							Utils::safe_inc($Distribution['version'][$LongMPEGversionLookup[$head4]]);
							Utils::safe_inc($Distribution['padding'][intval($LongMPEGpaddingLookup[$head4])]);
							Utils::safe_inc($Distribution['frequency'][$LongMPEGfrequencyLookup[$head4]]);
		...
		$info['mpeg']['audio']['sample_rate']  = Utils::array_max($Distribution['frequency'], true);

		$info['audio']['bitrate']      = $info['mpeg']['audio']['bitrate'];
		$info['audio']['bitrate_mode'] = $info['mpeg']['audio']['bitrate_mode'];
		$info['audio']['sample_rate']  = $info['mpeg']['audio']['sample_rate'];
		$info['audio']['dataformat']   = 'mp'.Utils::array_max($Distribution['layer'], true);
		$info['fileformat']            = $info['audio']['dataformat'];
			...
			if (($header{$SynchSeekOffset} == "\xFF") && ($header{($SynchSeekOffset + 1)} > "\xE0")) { // synch detected
				...
				if ($this->decodeMPEGaudioHeader($avdataoffset + $SynchSeekOffset, $dummy, true)) {
					...
					if ($BitrateHistogram) {
						...
						for ($current_segment = 0; $current_segment < $max_scan_segments; $current_segment++) {
							...
							while ($this->decodeMPEGaudioHeader($synchstartoffset, $dummy, false, false, $FastMode)) {
								...
								if (empty($dummy['mpeg']['audio']['framelength'])) {
									...
									Utils::safe_inc($info['mpeg']['audio']['bitrate_distribution'][$thisframebitrate]);
									Utils::safe_inc($info['mpeg']['audio']['stereo_distribution'][$dummy['mpeg']['audio']['channelmode']]);
									Utils::safe_inc($info['mpeg']['audio']['version_distribution'][$dummy['mpeg']['audio']['version']]);
						...
						$info['mpeg']['audio']['frame_count'] = Utils::CastAsInt($framecounter);
