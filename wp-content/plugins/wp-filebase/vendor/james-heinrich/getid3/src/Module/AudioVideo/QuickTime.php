...
class QuickTime extends \JamesHeinrich\GetID3\Module\Handler
{
	...
	public function Analyze() {
		...
		while ($offset < $info['avdataend']) {
			if (!Utils::intValueSupported($offset)) {
			...
			$atomsize = Utils::BigEndian2Int(substr($AtomHeader, 0, 4));
			$atomname = substr($AtomHeader, 4, 4);

			// 64-bit MOV patch by jlegateØktnc*com
			if ($atomsize == 1) {
				$atomsize = Utils::BigEndian2Int($this->fread(8));
			}
	...
	public function QuicktimeParseAtom($atomname, $atomsize, $atom_data, $baseoffset, &$atomHierarchy, $ParseAllPossibleAtoms) {
		...
		switch ($atomname) {
			...
			case "\x00\x00\x00\x05":
				$atomname = Utils::BigEndian2Int($atomname);
				$atom_structure['name'] = $atomname;
				$atom_structure['subatoms'] = $this->QuicktimeParseContainerAtom($atom_data, $baseoffset + 8, $atomHierarchy, $ParseAllPossibleAtoms);
				break;
			...
			case 'tvsn': // TV SeasoN
				if ($atom_parent == 'udta') {
					...
					$atom_structure['data_length'] = Utils::BigEndian2Int(substr($atom_data, 0, 2));
					$atom_structure['language_id'] = Utils::BigEndian2Int(substr($atom_data, 2, 2));
					...
					if (substr($atom_data, 2, 2) == "\x10\xB5") {
						...
						while ($atomoffset < strlen($atom_data)) {
							$boxsmallsize = Utils::BigEndian2Int(substr($atom_data, $atomoffset,     2));
							...
							switch ($boxsmalltype) {
								case "\x10\xB5":
									...
									break;
								default:
									$info['warning'][] = 'Unknown QuickTime smallbox type: "'.preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $boxsmalltype).'" ('.trim(Utils::PrintHexBytes($boxsmalltype)).') at offset '.$baseoffset;
						...
						while ($atomoffset < strlen($atom_data)) {
							$boxsize = Utils::BigEndian2Int(substr($atom_data, $atomoffset, 4));
							...
							switch ($boxtype) {
								...
								case 'data':
									$atom_structure['version']   = Utils::BigEndian2Int(substr($boxdata,  0, 1));
									$atom_structure['flags_raw'] = Utils::BigEndian2Int(substr($boxdata,  1, 3));
										...
										case 21: // tmpo/cpil flag
											switch ($atomname) {
												...
												case 'pgap':
													...
													$atom_structure['data'] = Utils::BigEndian2Int(substr($boxdata, 8, 1));
													break;

												case 'tmpo':
													// 16-bit integer
													$atom_structure['data'] = Utils::BigEndian2Int(substr($boxdata, 8, 2));
													break;

												case 'disk':
												case 'trkn':
													// binary
													$num       = Utils::BigEndian2Int(substr($boxdata, 10, 2));
													$num_total = Utils::BigEndian2Int(substr($boxdata, 12, 2));
												...
												case 'gnre':
													...
													$atom_structure['data']    = Utils::LookupGenreName($GenreID - 1);
													break;

												case 'rtng':
													// 8-bit integer
													$atom_structure[$atomname] = Utils::BigEndian2Int(substr($boxdata, 8, 1));
												...
												case 'sfID':
													// 32-bit integer
													$atom_structure[$atomname] = Utils::BigEndian2Int(substr($boxdata, 8, 4));
												...
												case 'plID':
													// 64-bit integer
													$atom_structure['data'] = Utils::BigEndian2Int(substr($boxdata, 8, 8));
												...
												default:
													// 32-bit integer
													$atom_structure['data'] = Utils::BigEndian2Int(substr($boxdata, 8, 4));
											}
											break;
								...
								default:
									$info['warning'][] = 'Unknown QuickTime box type: "'.preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $boxtype).'" ('.trim(Utils::PrintHexBytes($boxtype)).') at offset '.$baseoffset;
									$atom_structure['data'] = $atom_data;
			...
			case 'play': // auto-PLAY atom
				$atom_structure['autoplay'] = (bool) Utils::BigEndian2Int(substr($atom_data,  0, 1));
			...
			case 'WLOC': // Window LOCation atom
				$atom_structure['location_x']  = Utils::BigEndian2Int(substr($atom_data,  0, 2));
				$atom_structure['location_y']  = Utils::BigEndian2Int(substr($atom_data,  2, 2));
				break;
			...
			case 'AllF': // play ALL Frames atom
				$atom_structure['data'] = Utils::BigEndian2Int($atom_data);
				break;
			...
			case 'cmvd': // Compressed MooV Data atom
				...
				$atom_structure['unCompressedSize'] = Utils::BigEndian2Int(substr($atom_data, 0, 4));
			...
			case 'rdrf': // Reference movie Data ReFerence atom
				...
				$atom_structure['reference_length']       = Utils::BigEndian2Int(substr($atom_data,  8, 4));
			...
			case 'rmqu': // Reference Movie QUality atom
				$atom_structure['movie_quality'] = Utils::BigEndian2Int($atom_data);
				break;
			...
			case 'rmcs': // Reference Movie Cpu Speed atom
				...
				$atom_structure['cpu_speed_rating'] = Utils::BigEndian2Int(substr($atom_data,  4, 2));
				break;
			...
			case 'rmvc': // Reference Movie Version Check atom
				...
				$atom_structure['gestalt_value_mask'] = Utils::BigEndian2Int(substr($atom_data,  8, 4));
				$atom_structure['gestalt_value']      = Utils::BigEndian2Int(substr($atom_data, 12, 4));
				$atom_structure['gestalt_check_type'] = Utils::BigEndian2Int(substr($atom_data, 14, 2));
				break;
			...
			case 'rmcd': // Reference Movie Component check atom
				...
				$atom_structure['component_flags_raw']    = Utils::BigEndian2Int(substr($atom_data, 16, 4));
				$atom_structure['component_flags_mask']   = Utils::BigEndian2Int(substr($atom_data, 20, 4));
				$atom_structure['component_min_version']  = Utils::BigEndian2Int(substr($atom_data, 24, 4));
				break;
			...
			case 'rmdr': // Reference Movie Data Rate atom
				$atom_structure['version']       = Utils::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']     = Utils::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['data_rate']     = Utils::BigEndian2Int(substr($atom_data,  4, 4));
			...
			case 'rmla': // Reference Movie Language Atom
				$atom_structure['version']     = Utils::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']   = Utils::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['language_id'] = Utils::BigEndian2Int(substr($atom_data,  4, 2));
			...
			case 'ptv ': // Print To Video - defines a movie's full screen mode
				...
				$atom_structure['reserved_1']        = Utils::BigEndian2Int(substr($atom_data, 2, 2)); // hardcoded: 0x0000
				$atom_structure['reserved_2']        = Utils::BigEndian2Int(substr($atom_data, 4, 2)); // hardcoded: 0x0000
				$atom_structure['slide_show_flag']   = Utils::BigEndian2Int(substr($atom_data, 6, 1));
				$atom_structure['play_on_open_flag'] = Utils::BigEndian2Int(substr($atom_data, 7, 1));
			...
			case 'stsd': // Sample Table Sample Description atom
				...
				for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
					...
					$stsdEntriesDataOffset += 4;
					$atom_structure['sample_description_table'][$i]['reserved']         = Utils::BigEndian2Int(substr($atom_data, $stsdEntriesDataOffset, 6));
					$stsdEntriesDataOffset += 6;
					$atom_structure['sample_description_table'][$i]['reference_index']  = Utils::BigEndian2Int(substr($atom_data, $stsdEntriesDataOffset, 2));
					$stsdEntriesDataOffset += 2;
					$atom_structure['sample_description_table'][$i]['data']             =                           substr($atom_data, $stsdEntriesDataOffset, ($atom_structure['sample_description_table'][$i]['size'] - 4 - 4 - 6 - 2));
					$stsdEntriesDataOffset += ($atom_structure['sample_description_table'][$i]['size'] - 4 - 4 - 6 - 2);

					$atom_structure['sample_description_table'][$i]['encoder_version']  = Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'],  0, 2));
					$atom_structure['sample_description_table'][$i]['encoder_revision'] = Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'],  2, 2));
					...
					switch ($atom_structure['sample_description_table'][$i]['encoder_vendor']) {
						...
						case "\x00\x00\x00\x00":
							// audio tracks
							$atom_structure['sample_description_table'][$i]['audio_channels']       =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'],  8,  2));
							$atom_structure['sample_description_table'][$i]['audio_bit_depth']      =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 10,  2));
							$atom_structure['sample_description_table'][$i]['audio_compression_id'] =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 12,  2));
							$atom_structure['sample_description_table'][$i]['audio_packet_size']    =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 14,  2));
							$atom_structure['sample_description_table'][$i]['audio_sample_rate']    = Utils::FixedPoint16_16(substr($atom_structure['sample_description_table'][$i]['data'], 16,  4));
	...
	$info['quicktime']['video']['resolution_y'] = $info['video']['resolution_y'];
						...
						default:
							switch ($atom_structure['sample_description_table'][$i]['data_format']) {
								...
								default:
									// video atom
									$atom_structure['sample_description_table'][$i]['video_temporal_quality']  =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'],  8,  4));
									$atom_structure['sample_description_table'][$i]['video_spatial_quality']   =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 12,  4));
									$atom_structure['sample_description_table'][$i]['video_frame_width']       =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 16,  2));
									$atom_structure['sample_description_table'][$i]['video_frame_height']      =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 18,  2));
									$atom_structure['sample_description_table'][$i]['video_resolution_x']      = Utils::FixedPoint16_16(substr($atom_structure['sample_description_table'][$i]['data'], 20,  4));
									$atom_structure['sample_description_table'][$i]['video_resolution_y']      = Utils::FixedPoint16_16(substr($atom_structure['sample_description_table'][$i]['data'], 24,  4));
									$atom_structure['sample_description_table'][$i]['video_data_size']         =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 28,  4));
									$atom_structure['sample_description_table'][$i]['video_frame_count']       =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 32,  2));
									$atom_structure['sample_description_table'][$i]['video_encoder_name_len']  =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 34,  1));
									$atom_structure['sample_description_table'][$i]['video_encoder_name']      =                             substr($atom_structure['sample_description_table'][$i]['data'], 35, $atom_structure['sample_description_table'][$i]['video_encoder_name_len']);
									$atom_structure['sample_description_table'][$i]['video_pixel_color_depth'] =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 66,  2));
									$atom_structure['sample_description_table'][$i]['video_color_table_id']    =   Utils::BigEndian2Int(substr($atom_structure['sample_description_table'][$i]['data'], 68,  2));
			...
			case 'stts': // Sample Table Time-to-Sample atom
				...
				for ($i = 0; $i < $max_stts_entries_to_scan; $i++) {
					$atom_structure['time_to_sample_table'][$i]['sample_count']    = Utils::BigEndian2Int(substr($atom_data, $sttsEntriesDataOffset, 4));
					$sttsEntriesDataOffset += 4;
					$atom_structure['time_to_sample_table'][$i]['sample_duration'] = Utils::BigEndian2Int(substr($atom_data, $sttsEntriesDataOffset, 4));
					$sttsEntriesDataOffset += 4;
			...
			case 'stss': // Sample Table Sync Sample (key frames) atom
				if ($ParseAllPossibleAtoms) {
					...
					for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
						$atom_structure['time_to_sample_table'][$i] = Utils::BigEndian2Int(substr($atom_data, $stssEntriesDataOffset, 4));
			...
			case 'stsc': // Sample Table Sample-to-Chunk atom
				if ($ParseAllPossibleAtoms) {
					...
					for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
						$atom_structure['sample_to_chunk_table'][$i]['first_chunk']        = Utils::BigEndian2Int(substr($atom_data, $stscEntriesDataOffset, 4));
						$stscEntriesDataOffset += 4;
						$atom_structure['sample_to_chunk_table'][$i]['samples_per_chunk']  = Utils::BigEndian2Int(substr($atom_data, $stscEntriesDataOffset, 4));
						$stscEntriesDataOffset += 4;
						$atom_structure['sample_to_chunk_table'][$i]['sample_description'] = Utils::BigEndian2Int(substr($atom_data, $stscEntriesDataOffset, 4));
						$stscEntriesDataOffset += 4;
					}
				}
				break;
			...
			case 'stsz': // Sample Table SiZe atom
				if ($ParseAllPossibleAtoms) {
					$atom_structure['version']        = Utils::BigEndian2Int(substr($atom_data,  0, 1));
					$atom_structure['flags_raw']      = Utils::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
					$atom_structure['sample_size']    = Utils::BigEndian2Int(substr($atom_data,  4, 4));
					$atom_structure['number_entries'] = Utils::BigEndian2Int(substr($atom_data,  8, 4));
					$stszEntriesDataOffset = 12;
					if ($atom_structure['sample_size'] == 0) {
						for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
							$atom_structure['sample_size_table'][$i] = Utils::BigEndian2Int(substr($atom_data, $stszEntriesDataOffset, 4));
			...
			case 'stco': // Sample Table Chunk Offset atom
				if ($ParseAllPossibleAtoms) {
					...
					for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
						$atom_structure['chunk_offset_table'][$i] = Utils::BigEndian2Int(substr($atom_data, $stcoEntriesDataOffset, 4));
			...
			case 'co64': // Chunk Offset 64-bit (version of "stco" that supports > 2GB files)
				if ($ParseAllPossibleAtoms) {
					...
					$stcoEntriesDataOffset = 8;
					for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
						$atom_structure['chunk_offset_table'][$i] = Utils::BigEndian2Int(substr($atom_data, $stcoEntriesDataOffset, 8));
						$stcoEntriesDataOffset += 8;
					}
				}
				break;
			...
			case 'dref': // Data REFerence atom
				...
				$drefDataOffset = 8;
				for ($i = 0; $i < $atom_structure['number_entries']; $i++) {
					$atom_structure['data_references'][$i]['size']                    = Utils::BigEndian2Int(substr($atom_data, $drefDataOffset, 4));
					$drefDataOffset += 4;
					$atom_structure['data_references'][$i]['type']                    =               substr($atom_data, $drefDataOffset, 4);
					$drefDataOffset += 4;
					$atom_structure['data_references'][$i]['version']                 = Utils::BigEndian2Int(substr($atom_data,  $drefDataOffset, 1));
					$drefDataOffset += 1;
					$atom_structure['data_references'][$i]['flags_raw']               = Utils::BigEndian2Int(substr($atom_data,  $drefDataOffset, 3)); // hardcoded: 0x0000
					$drefDataOffset += 3;
			...
			case 'gmin': // base Media INformation atom
				$atom_structure['version']                = Utils::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']              = Utils::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['graphics_mode']          = Utils::BigEndian2Int(substr($atom_data,  4, 2));
				$atom_structure['opcolor_red']            = Utils::BigEndian2Int(substr($atom_data,  6, 2));
				$atom_structure['opcolor_green']          = Utils::BigEndian2Int(substr($atom_data,  8, 2));
				$atom_structure['opcolor_blue']           = Utils::BigEndian2Int(substr($atom_data, 10, 2));
				$atom_structure['balance']                = Utils::BigEndian2Int(substr($atom_data, 12, 2));
				$atom_structure['reserved']               = Utils::BigEndian2Int(substr($atom_data, 14, 2));
				break;
			...
			case 'smhd': // Sound Media information HeaDer atom
				$atom_structure['version']                = Utils::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']              = Utils::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['balance']                = Utils::BigEndian2Int(substr($atom_data,  4, 2));
				$atom_structure['reserved']               = Utils::BigEndian2Int(substr($atom_data,  6, 2));
				break;
			...
			case 'mdhd': // MeDia HeaDer atom
				$atom_structure['version']               = Utils::BigEndian2Int(substr($atom_data,  0, 1));
				$atom_structure['flags_raw']             = Utils::BigEndian2Int(substr($atom_data,  1, 3)); // hardcoded: 0x0000
				$atom_structure['creation_time']         = Utils::BigEndian2Int(substr($atom_data,  4, 4));
				$atom_structure['modify_time']           = Utils::BigEndian2Int(substr($atom_data,  8, 4));
				$atom_structure['time_scale']            = Utils::BigEndian2Int(substr($atom_data, 12, 4));
				$atom_structure['duration']              = Utils::BigEndian2Int(substr($atom_data, 16, 4));
				$atom_structure['language_id']           = Utils::BigEndian2Int(substr($atom_data, 20, 2));
				$atom_structure['quality']               = Utils::BigEndian2Int(substr($atom_data, 22, 2));
			...
			case 'pnot': // Preview atom
				$atom_structure['modification_date']      = Utils::BigEndian2Int(substr($atom_data,  0, 4)); // "standard Macintosh format"
				$atom_structure['version_number']         = Utils::BigEndian2Int(substr($atom_data,  4, 2)); // hardcoded: 0x00
				$atom_structure['atom_type']              =               substr($atom_data,  6, 4);        // usually: 'PICT'
				$atom_structure['atom_index']             = Utils::BigEndian2Int(substr($atom_data, 10, 2)); // usually: 0x01

				$atom_structure['modification_date_unix'] = Utils::DateMac2Unix($atom_structure['modification_date']);
				break;


			case 'crgn': // Clipping ReGioN atom
				$atom_structure['region_size']   = Utils::BigEndian2Int(substr($atom_data,  0, 2)); // The Region size, Region boundary box,
				$atom_structure['boundary_box']  = Utils::BigEndian2Int(substr($atom_data,  2, 8)); // and Clipping region data fields
				$atom_structure['clipping_data'] =               substr($atom_data, 10);           // constitute a QuickDraw region.
				break;


			case 'load': // track LOAD settings atom
				$atom_structure['preload_start_time'] = Utils::BigEndian2Int(substr($atom_data,  0, 4));
				$atom_structure['preload_duration']   = Utils::BigEndian2Int(substr($atom_data,  4, 4));
				$atom_structure['preload_flags_raw']  = Utils::BigEndian2Int(substr($atom_data,  8, 4));
				$atom_structure['default_hints_raw']  = Utils::BigEndian2Int(substr($atom_data, 12, 4));
			...
			case 'ssrc': // non-primary SouRCe atom
				for ($i = 0; $i < strlen($atom_data); $i += 4) {
					@$atom_structure['track_id'][] = Utils::BigEndian2Int(substr($atom_data, $i, 4));
				}
				break;
			...
			case 'elst': // Edit LiST atom
				...
				$atom_structure['number_entries'] = Utils::BigEndian2Int(substr($atom_data,  4, 4));
				for ($i = 0; $i < $atom_structure['number_entries']; $i++ ) {
					$atom_structure['edit_list'][$i]['track_duration'] =   Utils::BigEndian2Int(substr($atom_data, 8 + ($i * 12) + 0, 4));
					$atom_structure['edit_list'][$i]['media_time']     =   Utils::BigEndian2Int(substr($atom_data, 8 + ($i * 12) + 4, 4));
					$atom_structure['edit_list'][$i]['media_rate']     = Utils::FixedPoint16_16(substr($atom_data, 8 + ($i * 12) + 8, 4));
				}
				break;
			...
			case 'ctab': // Color TABle atom
				$atom_structure['color_table_seed']   = Utils::BigEndian2Int(substr($atom_data,  0, 4)); // hardcoded: 0x00000000
				$atom_structure['color_table_flags']  = Utils::BigEndian2Int(substr($atom_data,  4, 2)); // hardcoded: 0x8000
				$atom_structure['color_table_size']   = Utils::BigEndian2Int(substr($atom_data,  6, 2)) + 1;
				for ($colortableentry = 0; $colortableentry < $atom_structure['color_table_size']; $colortableentry++) {
					$atom_structure['color_table'][$colortableentry]['alpha'] = Utils::BigEndian2Int(substr($atom_data, 8 + ($colortableentry * 8) + 0, 2));
					$atom_structure['color_table'][$colortableentry]['red']   = Utils::BigEndian2Int(substr($atom_data, 8 + ($colortableentry * 8) + 2, 2));
					$atom_structure['color_table'][$colortableentry]['green'] = Utils::BigEndian2Int(substr($atom_data, 8 + ($colortableentry * 8) + 4, 2));
					$atom_structure['color_table'][$colortableentry]['blue']  = Utils::BigEndian2Int(substr($atom_data, 8 + ($colortableentry * 8) + 6, 2));
				}
				break;
			...
			case 'mvhd': // MoVie HeaDer atom
				...
				$atom_structure['time_scale']         =   Utils::BigEndian2Int(substr($atom_data, 12, 4));
				$atom_structure['duration']           =   Utils::BigEndian2Int(substr($atom_data, 16, 4));
				$atom_structure['preferred_rate']     = Utils::FixedPoint16_16(substr($atom_data, 20, 4));
				$atom_structure['preferred_volume']   =   Utils::FixedPoint8_8(substr($atom_data, 24, 2));
				$atom_structure['reserved']           =                             substr($atom_data, 26, 10);
				$atom_structure['matrix_a']           = Utils::FixedPoint16_16(substr($atom_data, 36, 4));
				$atom_structure['matrix_b']           = Utils::FixedPoint16_16(substr($atom_data, 40, 4));
				$atom_structure['matrix_u']           =  Utils::FixedPoint2_30(substr($atom_data, 44, 4));
				$atom_structure['matrix_c']           = Utils::FixedPoint16_16(substr($atom_data, 48, 4));
				$atom_structure['matrix_d']           = Utils::FixedPoint16_16(substr($atom_data, 52, 4));
				$atom_structure['matrix_v']           =  Utils::FixedPoint2_30(substr($atom_data, 56, 4));
				$atom_structure['matrix_x']           = Utils::FixedPoint16_16(substr($atom_data, 60, 4));
				$atom_structure['matrix_y']           = Utils::FixedPoint16_16(substr($atom_data, 64, 4));
				$atom_structure['matrix_w']           =  Utils::FixedPoint2_30(substr($atom_data, 68, 4));
				$atom_structure['preview_time']       =   Utils::BigEndian2Int(substr($atom_data, 72, 4));
				$atom_structure['preview_duration']   =   Utils::BigEndian2Int(substr($atom_data, 76, 4));
				$atom_structure['poster_time']        =   Utils::BigEndian2Int(substr($atom_data, 80, 4));
				$atom_structure['selection_time']     =   Utils::BigEndian2Int(substr($atom_data, 84, 4));
				$atom_structure['selection_duration'] =   Utils::BigEndian2Int(substr($atom_data, 88, 4));
				$atom_structure['current_time']       =   Utils::BigEndian2Int(substr($atom_data, 92, 4));
				$atom_structure['next_track_id']      =   Utils::BigEndian2Int(substr($atom_data, 96, 4));
			...
			case 'tkhd': // TracK HeaDer atom
				...
				$atom_structure['creation_time']       =   Utils::BigEndian2Int(substr($atom_data,  4, 4));
				$atom_structure['modify_time']         =   Utils::BigEndian2Int(substr($atom_data,  8, 4));
				$atom_structure['trackid']             =   Utils::BigEndian2Int(substr($atom_data, 12, 4));
				$atom_structure['reserved1']           =   Utils::BigEndian2Int(substr($atom_data, 16, 4));
				$atom_structure['duration']            =   Utils::BigEndian2Int(substr($atom_data, 20, 4));
				$atom_structure['reserved2']           =   Utils::BigEndian2Int(substr($atom_data, 24, 8));
				$atom_structure['layer']               =   Utils::BigEndian2Int(substr($atom_data, 32, 2));
				$atom_structure['alternate_group']     =   Utils::BigEndian2Int(substr($atom_data, 34, 2));
				$atom_structure['volume']              =   Utils::FixedPoint8_8(substr($atom_data, 36, 2));
				$atom_structure['reserved3']           =   Utils::BigEndian2Int(substr($atom_data, 38, 2));
// http://developer.apple.com/library/mac/#documentation/QuickTime/RM/MovieBasics/MTEditing/K-Chapter/11MatrixFunctions.html
// http://developer.apple.com/library/mac/#documentation/QuickTime/qtff/QTFFChap4/qtff4.html#//apple_ref/doc/uid/TP40000939-CH206-18737
				$atom_structure['matrix_a']            = Utils::FixedPoint16_16(substr($atom_data, 40, 4));
				$atom_structure['matrix_b']            = Utils::FixedPoint16_16(substr($atom_data, 44, 4));
				$atom_structure['matrix_u']            =  Utils::FixedPoint2_30(substr($atom_data, 48, 4));
				$atom_structure['matrix_c']            = Utils::FixedPoint16_16(substr($atom_data, 52, 4));
				$atom_structure['matrix_d']            = Utils::FixedPoint16_16(substr($atom_data, 56, 4));
				$atom_structure['matrix_v']            =  Utils::FixedPoint2_30(substr($atom_data, 60, 4));
				$atom_structure['matrix_x']            = Utils::FixedPoint16_16(substr($atom_data, 64, 4));
				$atom_structure['matrix_y']            = Utils::FixedPoint16_16(substr($atom_data, 68, 4));
				$atom_structure['matrix_w']            =  Utils::FixedPoint2_30(substr($atom_data, 72, 4));
				$atom_structure['width']               = Utils::FixedPoint16_16(substr($atom_data, 76, 4));
				$atom_structure['height']              = Utils::FixedPoint16_16(substr($atom_data, 80, 4));
			...
			case 'iods': // Initial Object DeScriptor atom
				...
				$offset = 0;
				$atom_structure['version']                =       Utils::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['flags_raw']              =       Utils::BigEndian2Int(substr($atom_data, $offset, 3));
				$offset += 3;
				$atom_structure['mp4_iod_tag']            =       Utils::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['length']                 = $this->quicktime_read_mp4_descr_length($atom_data, $offset);
				//$offset already adjusted by quicktime_read_mp4_descr_length()
				$atom_structure['object_descriptor_id']   =       Utils::BigEndian2Int(substr($atom_data, $offset, 2));
				$offset += 2;
				$atom_structure['od_profile_level']       =       Utils::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['scene_profile_level']    =       Utils::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['audio_profile_id']       =       Utils::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['video_profile_id']       =       Utils::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				$atom_structure['graphics_profile_level'] =       Utils::BigEndian2Int(substr($atom_data, $offset, 1));
				$offset += 1;
				...
				for ($i = 0; $i < $atom_structure['num_iods_tracks']; $i++) {
					$atom_structure['track'][$i]['ES_ID_IncTag'] =       Utils::BigEndian2Int(substr($atom_data, $offset, 1));
					$offset += 1;
					$atom_structure['track'][$i]['length']       = $this->quicktime_read_mp4_descr_length($atom_data, $offset);
					//$offset already adjusted by quicktime_read_mp4_descr_length()
					$atom_structure['track'][$i]['track_id']     =       Utils::BigEndian2Int(substr($atom_data, $offset, 4));
					$offset += 4;
				}
			...
			case 'ftyp': // FileTYPe (?) atom (for MP4 it seems)
				...
				$atom_structure['unknown_1'] = Utils::BigEndian2Int(substr($atom_data,  4, 4));
...
/* due to lack of known documentation, this is a kludge implementation. If you know of documentation on how mdat is properly structed, please send it to info@getid3.org */
				...
				while  (($chapter_string_length = Utils::BigEndian2Int(substr($atom_data, $mdat_offset, 2)))
			...
			case 'nsav': // NoSAVe atom
				...
				$atom_structure['data'] = Utils::BigEndian2Int(substr($atom_data,  0, 4));
				break;
			...
			case 'pano': // PANOrama track (seen on QTVR)
				$atom_structure['pano'] = Utils::BigEndian2Int(substr($atom_data,  0, 4));
				break;
			...
			case 'imgt': // IMaGe Track reference (kQTVRImageTrackRefType) (seen on QTVR)
				for ($i = 0; $i < ($atom_structure['size'] - 8); $i += 4) {
					$atom_structure['imgt'][] = Utils::BigEndian2Int(substr($atom_data, $i, 4));
				}
				break;
			...
			case 'data': // metaDATA atom
				...
				$atom_structure['unknown']  = Utils::BigEndian2Int(substr($atom_data, 4 + 2, 2));
				$atom_structure['data']     =                           substr($atom_data, 4 + 4);
				break;

			default:
				$info['warning'][] = 'Unknown QuickTime atom type: "'.preg_replace('#[^a-zA-Z0-9 _\\-]#', '?', $atomname).'" ('.trim(Utils::PrintHexBytes($atomname)).') at offset '.$baseoffset;
				$atom_structure['data'] = $atom_data;
				break;
		}
...
//echo 'QuicktimeParseContainerAtom('.substr($atom_data, 4, 4).') @ '.$baseoffset.'<br><br>';
		...
		while ($subatomoffset < strlen($atom_data)) {
			$subatomsize = Utils::BigEndian2Int(substr($atom_data, $subatomoffset + 0, 4));
	...
	public function QuicktimeLanguageLookup($languageid) {
		...
		if (($languageid > 138) && ($languageid < 32767)) {
			...
			*/
			...
			$QuicktimeLanguageLookup[$languageid] = ID3v2::LanguageLookup($iso_language_id);
		}
...
//echo Utils::PrintHexBytes(substr($atom_data, $offset, 4)).'<br>';
