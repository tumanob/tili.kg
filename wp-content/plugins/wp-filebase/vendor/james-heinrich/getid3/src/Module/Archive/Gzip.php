	...
	public function Analyze() {
		...
		for ($i = 0; $i < $num_members; $i++) {
			...
			if ($thisInfo['flags']['extra']) {
				...
				$xlen = Utils::LittleEndian2Int($w_xlen);
				$fpointer += 2;
				...
				while (true) {
					...
					if (($si1 == 0x41) && ($si2 == 0x70)) {
						...
						$xsublen = Utils::LittleEndian2Int($w_xsublen);
						$idx += 2;
			...
			if ($thisInfo['flags']['crc16']) {
				...
				$thisInfo['crc16'] = Utils::LittleEndian2Int($w_crc);
				$fpointer += 2;
			}
			// bit 0 - FLG.FTEXT
			...
			$thisInfo['crc32']    = Utils::LittleEndian2Int(substr($buff, strlen($buff) - 8, 4));
			$thisInfo['filesize'] = Utils::LittleEndian2Int(substr($buff, strlen($buff) - 4));
