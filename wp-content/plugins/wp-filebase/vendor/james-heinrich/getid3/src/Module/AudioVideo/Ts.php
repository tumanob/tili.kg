...
class Ts extends \JamesHeinrich\GetID3\Module\Handler
{
	...
	public function Analyze() {
		...
		$offset = 0;
		$info['ts']['packet']['sync'] = Utils::BigEndian2Int(substr($TSheader, $offset, 1)); $offset += 1;
		$pid_flags_raw                = Utils::BigEndian2Int(substr($TSheader, $offset, 2)); $offset += 2;
		$SAC_raw                      = Utils::BigEndian2Int(substr($TSheader, $offset, 1)); $offset += 1;
		...
		if ($info['ts']['packet']['flags']['adaption_field_exists']) {
			$AdaptionField_raw        = Utils::BigEndian2Int(substr($TSheader, $offset, 2)); $offset += 2;
			...
			if ($info['ts']['packet']['adaption']['flags']['pcr']) {
				$info['ts']['packet']['adaption']['raw']['pcr'] = Utils::BigEndian2Int(substr($TSheader, $offset, 6)); $offset += 6;
			}
			if ($info['ts']['packet']['adaption']['flags']['opcr']) {
				$info['ts']['packet']['adaption']['raw']['opcr'] = Utils::BigEndian2Int(substr($TSheader, $offset, 6)); $offset += 6;
			}
		}
