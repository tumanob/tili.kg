...
class Flac extends \JamesHeinrich\GetID3\Module\Handler
{
	...
	public function Analyze() {
		...
		if ($StreamMarker != self::syncword) {
			return $this->error('Expecting "'.Utils::PrintHexBytes(self::syncword).'" at offset '.$info['avdataoffset'].', found "'.Utils::PrintHexBytes($StreamMarker).'"');
		}
	...
	private function parseSTREAMINFO($BlockData) {
		...
		$streaminfo['max_frame_size']  = Utils::BigEndian2Int(substr($BlockData, 7, 3));

		$SRCSBSS                       = Utils::BigEndian2Bin(substr($BlockData, 10, 8));
		$streaminfo['sample_rate']     = Utils::Bin2Dec(substr($SRCSBSS,  0, 20));
		$streaminfo['channels']        = Utils::Bin2Dec(substr($SRCSBSS, 20,  3)) + 1;
	...
	private function parseAPPLICATION($BlockData) {
		...
		$ApplicationID = Utils::BigEndian2Int(substr($BlockData, 0, 4));
	...
	private function parseSEEKTABLE($BlockData) {
		...
		while ($offset < $BlockLength) {
			...
			if ($SampleNumberString == $placeholderpattern) {
				...
				// placeholder point
				Utils::safe_inc($info['flac']['SEEKTABLE']['placeholders'], 1);
				$offset += 10;

			} else {

				$SampleNumber                                        = Utils::BigEndian2Int($SampleNumberString);
				$info['flac']['SEEKTABLE'][$SampleNumber]['offset']  = Utils::BigEndian2Int(substr($BlockData, $offset, 8));
				$offset += 8;
				$info['flac']['SEEKTABLE'][$SampleNumber]['samples'] = Utils::BigEndian2Int(substr($BlockData, $offset, 2));
				$offset += 2;
	...
	private function parseCUESHEET($BlockData) {
		...
		$offset += 128;
		$info['flac']['CUESHEET']['lead_in_samples']      =         Utils::BigEndian2Int(substr($BlockData, $offset, 8));
		$offset += 8;
		$info['flac']['CUESHEET']['flags']['is_cd']       = (bool) (Utils::BigEndian2Int(substr($BlockData, $offset, 1)) & 0x80);
		$offset += 1;

		$offset += 258; // reserved

		$info['flac']['CUESHEET']['number_tracks']        =         Utils::BigEndian2Int(substr($BlockData, $offset, 1));
		$offset += 1;

		for ($track = 0; $track < $info['flac']['CUESHEET']['number_tracks']; $track++) {
			$TrackSampleOffset = Utils::BigEndian2Int(substr($BlockData, $offset, 8));
			$offset += 8;
			$TrackNumber       = Utils::BigEndian2Int(substr($BlockData, $offset, 1));
			$offset += 1;
			...
			$TrackFlagsRaw                                                             = Utils::BigEndian2Int(substr($BlockData, $offset, 1));
			$offset += 1;
			...
			$info['flac']['CUESHEET']['tracks'][$TrackNumber]['index_points']          = Utils::BigEndian2Int(substr($BlockData, $offset, 1));
			$offset += 1;

			for ($index = 0; $index < $info['flac']['CUESHEET']['tracks'][$TrackNumber]['index_points']; $index++) {
				$IndexSampleOffset = Utils::BigEndian2Int(substr($BlockData, $offset, 8));
				$offset += 8;
				$IndexNumber       = Utils::BigEndian2Int(substr($BlockData, $offset, 1));
				$offset += 1;
	...
	*/
	public function parsePICTURE() {
		...
		$descr_length              = Utils::BigEndian2Int($this->fread(4));
		if ($descr_length) {
			$picture['description'] = $this->fread($descr_length);
		}
		$picture['image_width']    = Utils::BigEndian2Int($this->fread(4));
		$picture['image_height']   = Utils::BigEndian2Int($this->fread(4));
		$picture['color_depth']    = Utils::BigEndian2Int($this->fread(4));
		$picture['colors_indexed'] = Utils::BigEndian2Int($this->fread(4));
		$picture['datalength']     = Utils::BigEndian2Int($this->fread(4));
