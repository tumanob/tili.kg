...
class WPFB_AdminDashboard {
	...
	static function WidgetTools() {
		...
		<?php foreach ($tools as $id => $tool) {
			?>
				<li id="wpfb-tool-<?php echo $id; ?>"><a href="<?php echo $tool['url']; ?>" <?php if (!empty($tool['confirm'])) { ?> onclick="return confirm('<?php echo $tool['confirm']; ?>')" <?php } ?> class="button admin-scheme-fill-2-hover"><?php echo WPFB_Admin::Icon($tool['icon'], 32);
	...
	static function WidgetGetStarted() {
		...
		<div id="wpfb-how-start">
			<ul>
				<li><a href="<?php echo esc_attr(admin_url("admin.php?page=wpfilebase_cats#addcat")); ?>"><?php _e('Create a Category', 'wp-filebase') ?></a></li>
		...
		<?php if (!get_user_option(WPFB_OPT_NAME . '_hide_how_start')) { ?>
				<p style="text-align: right"><a href="<?php echo esc_attr(add_query_arg('wpfb-hide-how-start', '1')) ?>"><?php _e('Move this message to the bottom (dismiss)', 'wp-filebase') ?></a></p>
		<?php } ?>
		</div>
		...
		<?php if (strpos($_SERVER['SERVER_PROTOCOL'], 'HTTPS') === false) { ?><img src="http://fabi.me/misc/wpfb_icon.gif?lang=<?php if (defined('WPLANG')) {
				echo WPLANG;
			...
			<p><a href="<?php echo admin_url('plugins.php?wpfb-uninstall=1') ?>" class="button" onclick="return confirm('You will be brough to the Plugin page. When you disable WP-Filebase there, you loose all your data.')"><?php _e('Initiate a complete uninstall', 'wp-filebase') ?></a></p>
