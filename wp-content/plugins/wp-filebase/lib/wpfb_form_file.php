...
<form enctype="multipart/form-data" name="<?php echo $action ?>" id="<?php echo $action ?>" method="post" action="<?php echo $form_url ?>" class="validate">
	...
	if (!$in_widget) {
		if ($in_editor) {
			...
			echo "<h2>" . $title . ' ';
			if (!$update) {
				?><a style="font-style:normal; white-space: nowrap;" href="<?php echo add_query_arg('exform', ($exform ? '0' : '1'), admin_url('admin.php?page=wpfilebase_files')) . '#' . $action; ?>" class="add-new-h2"><?php _e($exform ? 'Simple Form' : 'Extended Form', 'wp-filebase') ?></a> <?php
			}
		...
		<tr class="form-field">
			...
			<td><input name="file_author" id="file_author" type="text" value="<?php echo esc_attr(!empty($file->file_author) ? $file->file_author : WPFB_Core::$settings->default_author); ?>" size="<?php echo ($in_editor || $in_widget) ? 20 : 40 ?>" /></td>
				...
				<td><?php
					...
					if ($file != null && $file->file_date)
						$comment->comment_date = $file->file_date;
					?><div class="wpfilebase-date-edit"><?php touch_time(1, 0); ?></div></td>
			</tr>
			<tr class="form-field">
					<?php } ?>
