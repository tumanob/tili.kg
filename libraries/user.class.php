...
abstract class EfrontUser
{
    ...
    public static function createUser($userProperties, $users = array(), $addToDefaultGroup = true)
    {
        ...
        isset($userProperties['password']) ? $passwordNonTransformed = $userProperties['password']
                : $passwordNonTransformed = $userProperties['login'];
        if ($userProperties['password'] != 'ldap') {
            !isset($userProperties['password'])
                    ? $userProperties['password'] = EfrontUser::createPassword($userProperties['login'])
                    : $userProperties['password'] = self :: createPassword($userProperties['password']);
        ...
        !isset($userProperties['languages_NAME'])
                ? $userProperties['languages_NAME'] = $GLOBALS['configuration']['default_language']
                : null; //If language is not specified, use default language
        !isset($userProperties['active']) || $userProperties['active'] == "" ? $userProperties['active'] = 0
                : null; // 0 means inactive, 1 means active
        !isset($userProperties['pending']) ? $userProperties['pending'] = 0
                : null; // 0 means not pending, 1 means pending
        !isset($userProperties['timestamp']) || $userProperties['timestamp'] == ""
                ? $userProperties['timestamp'] = time() : null;
    ...
    public function setLoginType($loginType = 'normal', $password = '')
    {
        ...
        if ($loginType == 'ldap' && $this->user['password'] != 'ldap') {
            ...
            !$password ? $password = EfrontUser::createPassword($this->user['login'])
                    : null; //If a password is not specified, use the user's login name
    ...
    public function addGroups($groupIds)
    {
        ...
        foreach ($groupIds as $key => $groupId) {
            if (eF_checkParameter($groupId, 'id') && !isset($this->groups[$groupId])) {
                ...
                $group->addUsers($this->user['login'], $this->user['user_types_ID'] ? $this->user['user_types_ID']
                                                             : $this->user['user_type']);
    ...
    public function getModules()
    {
        ...
        $modules = array();
        isset($_SESSION['s_lesson_user_type']) && $_SESSION['s_lesson_user_type']
                ? $user_type = $_SESSION['s_lesson_user_type'] : $user_type = $this->getType();
    ...
    public function applyRoleOptions($role = false)
    {
        ...
        if ($role) {
            ...
            unserialize($result[0]['core_access']) ? $this->coreAccess = unserialize($result[0]['core_access'])
                    : $this->coreAccess = null;
            unserialize($result[0]['modules_access']) ? $this->modulesAccess = unserialize($result[0]['modules_access'])
                    : $this->modulesAccess = null;
        }
    }
    ...
    public static function getRoles($getNames = false)
    {
        ...
        if (sizeof($roles) > 0) {
            $getNames ? $roles = self :: $basicUserTypesTranslations + array_combine($roles['id'], $roles['name'])
                    : $roles = array_combine(self :: $basicUserTypes, self :: $basicUserTypes) + array_combine($roles['id'], $roles['basic_user_type']);
        } else {
            $getNames ? $roles = self :: $basicUserTypesTranslations
                    : $roles = array_combine(self :: $basicUserTypes, self :: $basicUserTypes);
        }
        return $roles;
    }
    ...
    public static function convertDatabaseResultToUserObjects($result)
    {
        ...
        $userObjects = array();
        foreach ($result as $value) {
            $userObjects[$value['login']] = EfrontUserFactory::factory($value, false, ($value['role']
...
abstract class EfrontLessonUser extends EfrontUser
{
    ...
    public function getNonLessons($returnObjects = false)
    {
        ...
        sizeof($userLessons) > 0 ? $sql = "active = 1 and id not in (" . implode(",", $userLessons['lessons_ID']) . ")"
                : $sql = 'active = 1';
        if ($returnObjects) {
...
*/
        ...
        foreach ($courses as $course) {
            if ($certificateInfo = unserialize($course['issued_certificate'])) {
                ...
                $certificates[] = array("courses_ID" => $course['id'],
                                        ...
                                        "expiration_date" => ($course['certificate_expiration'])
                                                ? $expire_certificateTimestamp : _NEVER);
            }
        }
        return $certificates;
    }
    ...
    public static function getLessonsRoles($getNames = false)
    {
        ...
        if (sizeof($roles) > 0) {
            $getNames
                    ? $roles = array('student' => _STUDENT, 'professor' => _PROFESSOR) + array_combine($roles['id'], $roles['name'])
                    : $roles = array('student' => 'student', 'professor' => 'professor') + array_combine($roles['id'], $roles['basic_user_type']);
        } else {
            $getNames ? $roles = array('student' => _STUDENT, 'professor' => _PROFESSOR)
                    : $roles = array('student' => 'student', 'professor' => 'professor');
        }
        return $roles;
    }
     ...
     * @access public
     */
    public function getCourseRoles($courseId)
    { //PROTONC
     ...
     * @static
     */
    public static function getAvailableRoles($getNames = false)
    { //PROTONC
        $roles = eF_getTableDataFlat("user_types", "*", "active=1 AND basic_user_type!='administrator'"); //Get available roles
        if (sizeof($roles) > 0) {
            $getNames
