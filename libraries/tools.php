...
function formatStaticText($value)
{
  ...
  <div>
   ...
   <a href = "javascript:toggleEditor(\'data\',\'simpleEditor\');" id = "###editor_id###">' . _TOGGLEHTMLEDITORMODE . '</a>
    ...
    switch ($value['name']) {
        case 'toggle_editor_code':
            $value['label'] = str_replace('###editor_id###', $value['label'], $toggleEditorCode);
            break;
        default:
            break;
    }
...
{
    isset($_GET['limit']) && eF_checkParameter($_GET['limit'], 'uint') ? $limit = $_GET['limit']
            : $limit = G_DEFAULT_TABLE_SIZE;
...
function createConstraintsFromSortedTable()
{
    ...
    isset($_GET['offset']) && eF_checkParameter($_GET['offset'], 'int') ? $constraints['offset'] = $_GET['offset']
            : null;
    isset($_GET['limit']) && eF_checkParameter($_GET['limit'], 'int') ? $constraints['limit'] = $_GET['limit']
            : $constraints['limit'] = G_DEFAULT_TABLE_SIZE;
    isset($_GET['sort']) && eF_checkParameter($_GET['sort'], 'alnum_with_spaces') ? $constraints['sort'] = $_GET['sort']
            : null;
    isset($_GET['filter']) ? $constraints['filter'] = $_GET['filter'] : null;
    isset($_GET['order']) && in_array($_GET['order'], array('asc', 'desc')) ? $constraints['order'] = $_GET['order']
...
function eF_getModuleMenu($modules, $menu_category)
{
    ...
    foreach ($modules as $module) {
        if ($menu_category != "current_lesson" || ($menu_category == "current_lesson" && $GLOBALS['currentLesson']->options[$module->className])) {
            ...
            isset($sidebarLinks[$menu_category]) ? $sidebarLinks = $sidebarLinks[$menu_category]
                    : $sidebarLinks = array();
