...
// $base sanity check.
if ( 'BASE' == $base )
	die( /*WP_I18N_BASE_ERROR*/'Ошибочная настройка в <code>wp-config.php</code>. Переменная <code>$base</code> имеет значение <code>BASE</code>, а должно быть, например, <code>/</code> или <code>/blogs/</code>.'/*/WP_I18N_BASE_ERROR*/ );
...
if ( !isset( $current_site ) || !isset( $current_blog ) ) {
	...
	if ( false !== strpos( $domain, ':' ) ) {
		if ( substr( $domain, -3 ) == ':80' ) {
			...
			wp_die( /*WP_I18N_NO_PORT_NUMBER*/'Поддержка сети возможна только при отсутствии номера порта в URL.'/*/WP_I18N_NO_PORT_NUMBER*/ );
		}
	}
	...
	if ( ! $blog_id ) {
		if ( defined( 'WP_INSTALLING' ) ) {
			...
			$msg = ! $wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) ? ' ' . /*WP_I18N_TABLES_MISSING*/'В базе данных отсутствуют таблицы.'/*/WP_I18N_TABLES_MISSING*/ : '';
			wp_die( /*WP_I18N_NO_BLOG*/'В системе нет сайта с таким названием.'/*/WP_I18N_NO_BLOG*/ . $msg );
		}
	}
}
