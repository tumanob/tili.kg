...
if (!is_file($path . "configuration.php")) { //If the configuration file does not exist, this is a fresh installation, so redirect to installation page
    is_file("install/index.php") ? header("location:install/index.php")
            : print('Failed locating configuration file <br/> Failed locating installation directory <br/> Please execute installation script manually <br/>');
    exit;
} else {
    /** Configuration file */
        ...
        if (isset($_SESSION['s_login']) && $_SESSION['s_login']) {
            try {
                ...
                if ($GLOBALS['configuration']['logout_redirect']) {
                    strpos($GLOBALS['configuration']['logout_redirect'], 'https://') === 0 || strpos($GLOBALS['configuration']['logout_redirect'], 'http://') === 0
            ...
            if ($GLOBALS['configuration']['logout_redirect']) {
                if ($GLOBALS['configuration']['logout_redirect'] == 'close') {
                    ...
                    strpos($GLOBALS['configuration']['logout_redirect'], 'http://') === 0
                            ? eF_redirect("" . $GLOBALS['configuration']['logout_redirect'])
                            : header("location:http://" . $GLOBALS['configuration']['logout_redirect']);
                }
            }
        } catch (EfrontUserException $e) {
...
if (isset($_GET['register_lessons'])) {
    if (!$_SESSION['s_login']) {
        ...
        if ($GLOBALS['configuration']['signup']) {
            // $message .= _OTHERWISEPLEASEREGISTER;    //  tumanob commented to use wordpress login and register options
        }
...
isset($_GET['ctg']) && $_GET['ctg'] == 'login' ? $postTarget = basename($_SERVER['PHP_SELF']) . "?ctg=login"
        : $postTarget = basename($_SERVER['PHP_SELF']) . "?index_page";
...
if (isset($_GET['ctg']) && ($_GET['ctg'] == "signup") && $configuration['signup']) {
    ...
    $form = new HTML_QuickForm("signup_register_personal_form", "post", basename($_SERVER['PHP_SELF']) . "?ctg=signup" . (isset($_GET['ldap'])
            ? '&ldap=1' : ''), "", "class = 'indexForm'", true);
    $form->removeAttribute('name');
    $form->registerRule('checkParameter', 'callback', 'eF_checkParameter'); //Register this rule for checking user input with our function, eF_checkParameter
    ...
    $form->addElement('text', 'login', _LOGIN, (isset($_GET['ldap']) ? 'class = "inputText inactiveElement" readonly'
                : 'class = "inputText"'));
    $form->addRule('login', _THEFIELD . ' ' . _LOGIN . ' ' . _ISMANDATORY, 'required', null, 'client');
